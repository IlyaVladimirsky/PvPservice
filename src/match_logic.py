...
logger.debug('{:*^50}'.format('New start'))
...
class MatchLogic:
    ...
    def __init__(self, clients_number=2, match_wait_time=1):
        """Init docs here"""
    ...
    async def create_match_request(self, **client_info):
        ...
        logger.debug('[create_match_request]: %s enter', client_info['nickname'])
        ...
        with await self._count_lock:
            ...
            if self._waiting_count == self.clients_number:
                ...
                logger.debug('[create_match_request]: %s unlocked event locker', client_info['nickname'])
                self._event_lock.set()

                await asyncio.sleep(0.00001)  # wait while other clients will be released from waiting
                logger.debug('[create_match_request]: %s locked event locker', client_info['nickname'])
        ...
        logger.debug('[create_match_request]: c=%d, eve=%s', self._waiting_count, self._event_lock.is_set())
        if not self._event_lock.is_set():
            try:
                ...
                logger.debug('[create_match_request]: %s - TimeoutError', client_info['nickname'])
